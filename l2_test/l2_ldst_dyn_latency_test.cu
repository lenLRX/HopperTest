...
void test_l2_bw_float4(int repeat) {
  ...
  double otsd = bw * avg_latency / 1.755 / 128 / (sm_count - 1);

  printf("test_l2_bw_float4 test time %5.2f bandwidth %6.2fG avg latency %6.2f outstanding %6.2f\n",
         device_time, bw, avg_latency, otsd);
}


__global__ void l2_float_stride_test(float* data, float* output_data, int64_t thread_stride,
                                     int64_t data_size, int64_t repeat, int* latency_output) {
...
void test_l2_bw_stride(int repeat, int thread_stride_byte) {
  ...
  l2_float_stride_test<<<sm_count, thread_num>>>(l2_dev, output_data, thread_stride_byte/sizeof(float), data_size, repeat, latency_output);
  ...
  double otsd = bw * avg_latency / 1.755 / 32 / (sm_count - 1);

  printf("test_l2_bw_stride test time %5.2f stride %4dB bandwidth %6.2fG avg latency %6.2f outstanding %6.2f\n",
         device_time, thread_stride_byte, bw, avg_latency, otsd);
}
