...
// N = 256
__global__ void
wgmma_m64n256k8_tf32_SS_test(float *gm_d, float *gm_a, float *gm_b,         
  ...
  using GMMA_t = SM90_64x256x8_F32TF32TF32_SS_TN<>;    
  GMMA_t::CRegisters reg_d;                                                          
                                                                                    
  float *sm_a = (float *)sm_buff;                                               
  for (int i = threadIdx.x; i < M * K / 2; i += blockDim.x) {                        
    sm_a[i] = gm_a[i];                                                               
  }                                                                                  
                                                                                     
  float *sm_b = (float *)(sm_buff + sizeof(float) * M * K / 2);                
...
void test_m64n256k8_tf32_SS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 8;
  ...
  cudaFuncSetAttribute(wgmma_m64n256k8_tf32_SS_test,
  ...
  while (1){
    ...
    wgmma_m64n256k8_tf32_SS_test<<<sm_count, 128, dyn_shared_size>>>(
...
__global__ void
wgmma_m64n256k8_tf32_RS_test(float *gm_d, float *gm_a, float *gm_b,         
                                      float *gm_c, uint8_t sm_layout,               
                                      uint64_t repeat_time) {                        
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 8;
  extern __shared__ char sm_buff[];                                                  
  using GMMA_t = SM90_64x256x8_F32TF32TF32_RS_TN<>;    
  GMMA_t::CRegisters reg_d;                                                          
  ...
  for (int i = 0; i < sizeof(reg_a)/sizeof(float); i += 1) {                        
    reinterpret_cast<float&>(reg_a[i]) = sm_a[i];
  }                                                                                  
...
void test_m64n256k8_tf32_RS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 8;
  ...
  cudaFuncSetAttribute(wgmma_m64n256k8_tf32_RS_test,
                       cudaFuncAttributeMaxDynamicSharedMemorySize,
                       dyn_shared_size);
  ...
  while (1){
    ...
    wgmma_m64n256k8_tf32_RS_test<<<sm_count, 128, dyn_shared_size>>>(
        mat_d_dev, mat_a_dev, mat_b_dev, mat_c_dev, 0, repeat_time);
    gpuErrchk(cudaEventRecord(stop_event, 0));
    gpuErrchk(cudaEventSynchronize(stop_event));
    gpuErrchk(cudaEventElapsedTime(&duration, start_event, stop_event));
...
// N = 128
__global__ void
wgmma_m64n128k8_tf32_SS_test(float *gm_d, float *gm_a, float *gm_b,         
				      float *gm_c, uint8_t sm_layout,               
				      uint64_t repeat_time) {                        
  constexpr int M = 64;
  constexpr int N = 128;
  constexpr int K = 8;
  extern __shared__ char sm_buff[];                                                  
  using GMMA_t = SM90_64x128x8_F32TF32TF32_SS_TN<>;    
  GMMA_t::CRegisters reg_d;                                                          
...
void test_m64n128k8_tf32_SS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 128;
  constexpr int K = 16;
  ...
  cudaFuncSetAttribute(wgmma_m64n128k8_tf32_SS_test,
		       cudaFuncAttributeMaxDynamicSharedMemorySize,
		       dyn_shared_size);
  ...
  while (1) {
    ...
    wgmma_m64n128k8_tf32_SS_test<<<sm_count, 128, dyn_shared_size>>>(
	mat_d_dev, mat_a_dev, mat_b_dev, mat_c_dev, 0, repeat_time);
    gpuErrchk(cudaEventRecord(stop_event, 0));
    gpuErrchk(cudaEventSynchronize(stop_event));
    gpuErrchk(cudaEventElapsedTime(&duration, start_event, stop_event));
...
__global__ void
wgmma_m64n128k8_tf32_RS_test(float *gm_d, float *gm_a, float *gm_b,         
				      float *gm_c, uint8_t sm_layout,               
				      uint64_t repeat_time) {                        
  constexpr int M = 64;
  constexpr int N = 128;
  constexpr int K = 8;
  extern __shared__ char sm_buff[];                                                  
  using GMMA_t = SM90_64x128x8_F32TF32TF32_RS_TN<>;    
  GMMA_t::CRegisters reg_d;                                                          
  GMMA_t::ARegisters reg_a;
...
void test_m64n128k8_tf32_RS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 128;
  constexpr int K = 8;
  ...
  cudaFuncSetAttribute(wgmma_m64n128k8_tf32_RS_test,
		       cudaFuncAttributeMaxDynamicSharedMemorySize,
		       dyn_shared_size);
  ...
  while (1){
    ...
    wgmma_m64n128k8_tf32_RS_test<<<sm_count, 128, dyn_shared_size>>>(
	mat_d_dev, mat_a_dev, mat_b_dev, mat_c_dev, 0, repeat_time);
    gpuErrchk(cudaEventRecord(stop_event, 0));
    gpuErrchk(cudaEventSynchronize(stop_event));
    gpuErrchk(cudaEventElapsedTime(&duration, start_event, stop_event));
...
// N = 64
__global__ void
wgmma_m64n64k8_tf32_SS_test(float *gm_d, float *gm_a, float *gm_b,         
				      float *gm_c, uint8_t sm_layout,               
				      uint64_t repeat_time) {                        
  constexpr int M = 64;
  constexpr int N = 64;
  constexpr int K = 8;
  extern __shared__ char sm_buff[];                                                  
  using GMMA_t = SM90_64x64x8_F32TF32TF32_SS_TN<>;    
  GMMA_t::CRegisters reg_d;                                                          
...
void test_m64n64k8_tf32_SS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 64;
  constexpr int K = 8;
  ...
  cudaFuncSetAttribute(wgmma_m64n64k8_tf32_SS_test,
		       cudaFuncAttributeMaxDynamicSharedMemorySize,
		       dyn_shared_size);
  ...
  while (1) {
    ...
    wgmma_m64n64k8_tf32_SS_test<<<sm_count, 128, dyn_shared_size>>>(
	mat_d_dev, mat_a_dev, mat_b_dev, mat_c_dev, 0, repeat_time);
    gpuErrchk(cudaEventRecord(stop_event, 0));
    gpuErrchk(cudaEventSynchronize(stop_event));
    gpuErrchk(cudaEventElapsedTime(&duration, start_event, stop_event));
...
__global__ void
wgmma_m64n64k8_tf32_RS_test(float *gm_d, float *gm_a, float *gm_b,         
				      float *gm_c, uint8_t sm_layout,               
				      uint64_t repeat_time) {                        
  constexpr int M = 64;
  constexpr int N = 64;
  constexpr int K = 8;
  extern __shared__ char sm_buff[];                                                  
  using GMMA_t = SM90_64x64x8_F32TF32TF32_RS_TN<>;    
  GMMA_t::CRegisters reg_d;                                                          
  GMMA_t::ARegisters reg_a;
...
void test_m64n64k8_tf32_RS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 64;
  constexpr int K = 8;
  ...
  cudaFuncSetAttribute(wgmma_m64n64k8_tf32_RS_test,
		       cudaFuncAttributeMaxDynamicSharedMemorySize,
		       dyn_shared_size);
  ...
  while (1) {
    ...
    wgmma_m64n64k8_tf32_RS_test<<<sm_count, 128, dyn_shared_size>>>(
	mat_d_dev, mat_a_dev, mat_b_dev, mat_c_dev, 0, repeat_time);
    gpuErrchk(cudaEventRecord(stop_event, 0));
    gpuErrchk(cudaEventSynchronize(stop_event));
    gpuErrchk(cudaEventElapsedTime(&duration, start_event, stop_event));
...
int main(int argc, char **argv) {
  ...
  if (n_size == "64") {
    if (a_scope == "ss") {
      test_m64n64k8_tf32_SS(argc, argv);
    }
    else if (a_scope == "rs") {
      test_m64n64k8_tf32_RS(argc, argv);
    }
  }

  if (n_size == "128") {
    if (a_scope == "ss") {
      test_m64n128k8_tf32_SS(argc, argv);
    }
    else if (a_scope == "rs") {
      test_m64n128k8_tf32_RS(argc, argv);
    }
  }

  if (n_size == "256") {
    if (a_scope == "ss") {
      test_m64n256k8_tf32_SS(argc, argv);
    }
    else if (a_scope == "rs") {
      test_m64n256k8_tf32_RS(argc, argv);
    }
  }
