...
template <> void init_data(uint8_t *data, int count, std::string method) {
  if (method == "uniform") {
    std::random_device rd;
    std::mt19937 gen(rd());
...
template <> void init_data(int8_t *data, int count, std::string method) {
  if (method == "uniform") {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, 255);
...
template <> void init_data(int32_t *data, int count, std::string method) {
  if (method == "uniform") {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, 255);
...
// N = 256
__global__ void
wgmma_m64n256k32_u8_SS_test(int32_t *gm_d, uint8_t *gm_a, uint8_t *gm_b,         
                                      int32_t *gm_c, uint8_t sm_layout,               
                                      uint64_t repeat_time) {                        
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 32;
  extern __shared__ char sm_buff[];                                                  
  using GMMA_t = SM90_64x256x32_S32U8U8_SS_TN_SATURATE;    
  GMMA_t::CRegisters reg_d;                                                          
                                                                                    
  uint8_t *sm_a = (uint8_t *)sm_buff;                                               
  for (int i = threadIdx.x; i < M * K; i += blockDim.x) {                        
    sm_a[i] = gm_a[i];                                                               
  }                                                                                  
                                                                                     
  uint8_t *sm_b = (uint8_t *)(sm_buff + sizeof(uint8_t) * M * K);                
...
void test_m64n256k32_u8_SS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 32;
  ...
  uint8_t *mat_a_host = new uint8_t[mat_a_size];
  init_data(mat_a_host, mat_a_size, init_method);

  uint8_t *mat_b_host = new uint8_t[mat_b_size];
  ...
  uint8_t *mat_a_dev = nullptr;
  gpuErrchk(cudaMalloc(&mat_a_dev, mat_a_size * sizeof(uint8_t)));
  gpuErrchk(cudaMemcpy(mat_a_dev, mat_a_host, mat_a_size * sizeof(uint8_t),
                       cudaMemcpyHostToDevice));

  uint8_t *mat_b_dev = nullptr;
  gpuErrchk(cudaMalloc(&mat_b_dev, mat_b_size * sizeof(uint8_t)));
  gpuErrchk(cudaMemcpy(mat_b_dev, mat_b_host, mat_b_size * sizeof(uint8_t),
                       cudaMemcpyHostToDevice));
  ...
  int dyn_shared_size =
      mat_a_size * sizeof(uint8_t) + mat_b_size * sizeof(uint8_t);

  cudaFuncSetAttribute(wgmma_m64n256k32_u8_SS_test,
  ...
  while (1){
    ...
    wgmma_m64n256k32_u8_SS_test<<<sm_count, 128, dyn_shared_size>>>(
...
__global__ void
wgmma_m64n256k32_u8_RS_test(int32_t *gm_d, uint8_t *gm_a, uint8_t *gm_b,         
                                      int32_t *gm_c, uint8_t sm_layout,               
                                      uint64_t repeat_time) {                        
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 32;
  extern __shared__ char sm_buff[];                                                  
  using GMMA_t = SM90_64x256x32_S32U8U8_RS_TN_SATURATE;    
...
void test_m64n256k32_u8_RS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 32;
  ...
  cudaFuncSetAttribute(wgmma_m64n256k32_u8_RS_test,
                       cudaFuncAttributeMaxDynamicSharedMemorySize,
                       dyn_shared_size);
  ...
  while (1){
    ...
    wgmma_m64n256k32_u8_RS_test<<<sm_count, 128, dyn_shared_size>>>(
        mat_d_dev, mat_a_dev, mat_b_dev, mat_c_dev, 0, repeat_time);
    gpuErrchk(cudaEventRecord(stop_event, 0));
    gpuErrchk(cudaEventSynchronize(stop_event));
    gpuErrchk(cudaEventElapsedTime(&duration, start_event, stop_event));
...
int main(int argc, char **argv) {
  ...
  if (n_size == "256") {
    if (a_scope == "ss") {
      test_m64n256k32_u8_SS(argc, argv);
    }
    else if (a_scope == "rs") {
      test_m64n256k32_u8_RS(argc, argv);
    }
  }
