...
wgmma_m64n256k16_fmix_SS_test(float *gm_d, __half2 *gm_a, __half2 *gm_b,         
				      ...
				      uint64_t busy_repeat,
				      uint64_t idle_repeat) {                        
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 16;
  ...
  for (uint64_t repeat_i = 0; repeat_i < repeat_time; ++repeat_i) {                  
    for (int bi = 0; bi < busy_repeat; ++bi) {
    ...
    for (int j = 0; j < idle_repeat; ++j) {
      #pragma unroll
      for (int ri = 0; ri < 128; ++ri) {
        reg_d[ri] = reg_d[ri] * 0.5;
      }
    }
  }
...
void test_m64n256k16_fmix_SS(int argc, char **argv) {
  ...
  std::string str_busy_time(argv[2]);
  int busy_repeat = std::stoi(str_busy_time);
  std::string str_idle_time(argv[3]);
  int idle_repeat = std::stoi(str_idle_time);
...

  {
    ...
    wgmma_m64n256k16_fmix_SS_test<<<sm_count, 128, dyn_shared_size>>>(
        mat_d_dev, mat_a_dev, mat_b_dev, mat_c_dev, 0, repeat_time, busy_repeat, idle_repeat);
...
int main(int argc, char **argv) {
  if (argc < 4) {
    std::cout << " usage ./test <init method> <busy_time> <idle_time>\n";
    return -1;
  }
