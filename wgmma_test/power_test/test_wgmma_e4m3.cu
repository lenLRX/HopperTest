...
template <> void init_data(__nv_fp8_storage_t *data, int count, std::string method) {
  if (method == "uniform") {
    ...
    for (int i = 0; i < count; ++i) {
      data[i] = __nv_cvt_float_to_fp8(dis(gen), __NV_SATFINITE, __NV_E4M3);
    }
  } else {
    for (int i = 0; i < count; ++i) {
      data[i] = __nv_cvt_float_to_fp8(0, __NV_SATFINITE, __NV_E4M3);
    }
  }
}
...
// N = 256
__global__ void
wgmma_m64n256k32_e4m3_SS_test(float *gm_d, __nv_fp8_storage_t *gm_a, __nv_fp8_storage_t *gm_b,         
  ...
  using GMMA_t = SM90_64x256x32_F32E4M3E4M3_SS_TN<>;    
...
void test_m64n256k32_e4m3_SS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 32;
  ...
  cudaFuncSetAttribute(wgmma_m64n256k32_e4m3_SS_test,
  ...
  while (1){
    ...
    wgmma_m64n256k32_e4m3_SS_test<<<sm_count, 128, dyn_shared_size>>>(
...
__global__ void
wgmma_m64n256k32_e4m3_RS_test(float *gm_d, __nv_fp8_storage_t *gm_a, __nv_fp8_storage_t *gm_b,         
                                      float *gm_c, uint8_t sm_layout,               
                                      uint64_t repeat_time) {                        
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 32;
  extern __shared__ char sm_buff[];                                                  
  using GMMA_t = SM90_64x256x32_F32E4M3E4M3_RS_TN<>;    
...
void test_m64n256k32_e4m3_RS(int argc, char **argv) {
  constexpr int M = 64;
  constexpr int N = 256;
  constexpr int K = 32;
  ...
  cudaFuncSetAttribute(wgmma_m64n256k32_e4m3_RS_test,
                       cudaFuncAttributeMaxDynamicSharedMemorySize,
                       dyn_shared_size);
  ...
  while (1){
    ...
    wgmma_m64n256k32_e4m3_RS_test<<<sm_count, 128, dyn_shared_size>>>(
        mat_d_dev, mat_a_dev, mat_b_dev, mat_c_dev, 0, repeat_time);
    gpuErrchk(cudaEventRecord(stop_event, 0));
    gpuErrchk(cudaEventSynchronize(stop_event));
    gpuErrchk(cudaEventElapsedTime(&duration, start_event, stop_event));
...
int main(int argc, char **argv) {
  ...
  if (n_size == "256") {
    if (a_scope == "ss") {
      test_m64n256k32_e4m3_SS(argc, argv);
    }
    else if (a_scope == "rs") {
      test_m64n256k32_e4m3_RS(argc, argv);
    }
  }
